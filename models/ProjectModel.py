    ...
    async def get_project_or_create_one(self , project_id:int) :    
        async with self.db_client() as session:
            async with session.begin():
                ...
                project = result.scalar_one_or_none()  # type: ignore
                if project is None :
                    project_rec = Project(
    ...
    async def get_all_projects(self ,page:int = 1 , page_size:int = 10) :
            ...
            async with self.db_client() as session:
                async with session.begin():
                    ...
                    total_documents = total_documents.scalar_one()  # type: ignore
                    ...
                    projects = (await session.execute(query)).scalars().all() # type: ignore 
