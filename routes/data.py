...
async def process_data(project_id: str, process_request: ProcessRequest, request: Request):  # Fixed: renamed ProcessRequest param and added Request param
    ...
    if process_request.file_id:
        ...
        if asset_model is None:
            return JSONResponse(
                ...
                content={
                    "message": ResponseSignal.FILES_ID_ERROR.value,
                }    
            )
        project_files_ids = {
    ...
    if len(project_files_ids) == 0:
        return JSONResponse(
            status_code= status.HTTP_404_NOT_FOUND,
            content={
                "message": ResponseSignal.NOT_FOUND_FILE.value,
            }    
        ) 
    ...
    for  asset_id ,file_id in project_files_ids.items():
        ...
        # Added: log warning and continue to next file  
        if file_content is None:
             logger.warning(f"File content not found for file_id: {file_id}")
             continue
